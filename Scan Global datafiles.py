...
Path_source = r'\\tx-app\E\Python filer\Scan Global filer'
Path_archive = r'\\tx-app\E\Python filer\Scan Global filer\Arkiv'
...
File_name_new = File_name + '_' + str(File_timestamp) + '.xlsx'
File_complete_new = Path_source + File_name_new

server_04 = "sqlsrv04"
db_ds = "BKI_Datastore"
params_ds = f"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server_04};DATABASE={db_ds};trusted_connection=yes"
...
try:
    if os.path.exists( File_complete): # Check if file exists
        ...
        Df_sg["Leveringsdato"] = pd.to_datetime(
            pd.to_numeric(Df_sg["Leveringsdato"], errors="coerce")
            ,origin="1899-12-30"
            ,unit="D"
            ,errors="coerce"
        )
        Df_sg["ETA AARHUS"] = pd.to_datetime(
            pd.to_numeric(Df_sg["ETA AARHUS"], errors="coerce")
            ,origin="1899-12-30"
            ,unit="D"
            ,errors="coerce"
        )
        ...
        Df_sg["Nettovægt"] = pd.to_numeric(
            Df_sg["Nettovægt"]
            ,errors="coerce"
        ).fillna(0).astype(int)
        Df_sg["Bruttovægt"] = pd.to_numeric(
            Df_sg["Bruttovægt"]
            ,errors="coerce"
        ).fillna(0).astype(int)
    ...
    Df_log['Note'] = 'Error: ' + str(e)[:425] + ' Filename: ' + File_name_new
